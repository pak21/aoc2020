...
cups = [None] + list(map(lambda i: Cup(i+1), range(max_cup)))

first = cups[starting_ids[0]]
prev = first
for i in starting_ids[1:]:
    cup = cups[i]
    prev.next_cup = cups[i]
    prev = cup
...
for m in range(moves):
    ...
    after = target.next_cup
    target.next_cup = picked_up
    picked_up.next_cup.next_cup.next_cup = after
